...
internal class DefaultGetUrlTask @Inject constructor(
    ...
    private suspend fun doRequestWithCache(url: String, validityDurationInMillis: Long, strict: Boolean): String {
        // Get data from cache
        val cached = rawCacheStore.get(url)
        val dataFromCache = cached?.data
        val isCacheValid = cached != null && Date().time < cached.lastUpdatedTimestamp + validityDurationInMillis
        ...
        // Store cache
        rawCacheStore.put(url, data, Date().time)
