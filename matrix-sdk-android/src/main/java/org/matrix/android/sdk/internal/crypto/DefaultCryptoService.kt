...
//    val gossipingBuffer = mutableListOf<Event>()
    ...
    override suspend fun getMyCryptoDevice(): CryptoDeviceInfo = withContext(coroutineDispatchers.io) {
        myDeviceInfoHolder.get().myDevice
    }

    override suspend fun fetchDevicesList(): List<DeviceInfo> = withContext(coroutineDispatchers.io) {
    ...
    override fun exportSecrets(): Result<String> {
        return Result.failure(UnsupportedOperationException("Secret export is not supported by the libce crypto implementation"))
    }
...
//    }
    ...
    override suspend fun getCryptoDeviceInfo(userId: String): List<CryptoDeviceInfo> = withContext(coroutineDispatchers.io) {
        cryptoStore.getUserDeviceList(userId).orEmpty()
    }
//
...
//    }
    ...
    override suspend fun getCryptoDeviceInfoList(userId: String): List<CryptoDeviceInfo> = withContext(coroutineDispatchers.io) {
        cryptoStore.getUserDeviceList(userId).orEmpty()
    }
//
