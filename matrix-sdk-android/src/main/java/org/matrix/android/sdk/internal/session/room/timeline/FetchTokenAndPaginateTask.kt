...
internal class DefaultFetchTokenAndPaginateTask @Inject constructor(
    ...
    override suspend fun execute(params: FetchTokenAndPaginateTask.Params): TokenChunkEventPersistor.Result {
        ...
        val fromToken = (if (params.direction == PaginationDirection.FORWARDS) response.end else response.start)
                ?: throw IllegalStateException("No token found")
        database.awaitDbTransaction(dispatcher) {
            val chunkId = stores.chunk.findChunkIdIncludingEvent(params.roomId, params.lastKnownEventId)
            if (chunkId != null) {
                if (params.direction == PaginationDirection.FORWARDS) {
                    stores.chunk.updateNextToken(chunkId, fromToken)
                } else {
                    stores.chunk.updatePrevToken(chunkId, fromToken)
                }
            }
        }
        return paginationTask.execute(
                PaginationTask.Params(
