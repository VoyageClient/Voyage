...
class PalettePickerDialogFragment : DialogFragment() {
    ...
    // One unbroken bar: the swatches share the row's width, so a 3-color palette gets wider cells
    // than an 8-color one and both fill the same span.
    private fun swatchStrip(context: Context, colors: List<Int>): View {
        val height = (32 * resources.displayMetrics.density).toInt()
        return LinearLayout(context).apply {
            orientation = LinearLayout.HORIZONTAL
            colors.forEachIndexed { index, color ->
                addView(swatch(context, color, index + 1), LinearLayout.LayoutParams(0, height, 1f))
            }
        }
    }
