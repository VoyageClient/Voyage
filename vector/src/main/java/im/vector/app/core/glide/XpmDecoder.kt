...
internal val XPM_MAGIC_BYTES = "/* XPM */".toByteArray(Charsets.US_ASCII)
...
internal fun decodeXpmToBitmap(source: InputStream): Bitmap? {
...
internal class XpmDecoder(private val bitmapPool: BitmapPool) : ResourceDecoder<InputStream, Bitmap> {
    ...
    override fun handles(source: InputStream, options: Options): Boolean {
        ...
        source.mark(XPM_MAGIC_BYTES.size)
        return try {
            val buf = ByteArray(XPM_MAGIC_BYTES.size)
            val read = source.read(buf)
            read == XPM_MAGIC_BYTES.size && buf.contentEquals(XPM_MAGIC_BYTES)
        } finally {
            source.reset()
        }
    }
    ...
    override fun decode(source: InputStream, width: Int, height: Int, options: Options): Resource<Bitmap>? {
        return decodeXpmToBitmap(source)?.let { BitmapResource.obtain(it, bitmapPool) }
    }
}
