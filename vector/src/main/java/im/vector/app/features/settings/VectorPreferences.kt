...
//        const val SETTINGS_SECURE_BACKUP_RESET_PREFERENCE_KEY = "SETTINGS_SECURE_BACKUP_RESET_PREFERENCE_KEY"
        ...
        const val SETTINGS_LINK_PREVIEW_MODE_KEY = "SETTINGS_LINK_PREVIEW_MODE_KEY"
    ...
    /**
     * In which rooms the previews of the links we send are generated by fetching the page on this device,
     * rather than asking the homeserver for them. A room may override the account-wide mode.
     */
    fun getLinkPreviewMode(roomId: String? = null): LinkPreviewMode {
        roomId?.let { getRoomLinkPreviewOverride(it) }?.let { return it }
        return LinkPreviewMode.fromValue(defaultPrefs.getString(SETTINGS_LINK_PREVIEW_MODE_KEY, LinkPreviewMode.ALWAYS.value))
    }

    /** null means the room inherits the account-wide mode. */
    fun getRoomLinkPreviewOverride(roomId: String): LinkPreviewMode? {
        return defaultPrefs.getString(roomLinkPreviewKey(roomId), null)?.let { LinkPreviewMode.fromValue(it) }
    }

    fun setRoomLinkPreviewOverride(roomId: String, mode: LinkPreviewMode?) {
        defaultPrefs.edit {
            if (mode == null) remove(roomLinkPreviewKey(roomId)) else putString(roomLinkPreviewKey(roomId), mode.value)
        }
    }

    private fun roomLinkPreviewKey(roomId: String) = "${SETTINGS_LINK_PREVIEW_MODE_KEY}_$roomId"
