...
//        const val SETTINGS_SECURE_BACKUP_RESET_PREFERENCE_KEY = "SETTINGS_SECURE_BACKUP_RESET_PREFERENCE_KEY"
        ...
        const val SETTINGS_PROFILE_BANNER_PREFERENCE_KEY = "SETTINGS_PROFILE_BANNER_PREFERENCE_KEY"
        ...
        // Also read by the SDK, see DefaultLightweightSettingsStorage.
        const val SETTINGS_LINK_PREVIEW_ENCRYPTED_KEY = "SETTINGS_LINK_PREVIEW_ENCRYPTED_KEY"
        const val SETTINGS_LINK_PREVIEW_UNENCRYPTED_KEY = "SETTINGS_LINK_PREVIEW_UNENCRYPTED_KEY"
        const val SETTINGS_LINK_PREVIEW_SOURCE_KEY = "SETTINGS_LINK_PREVIEW_SOURCE_KEY"
        ...
        const val SETTINGS_LABS_ENCRYPT_ACCOUNT_DATA = "SETTINGS_LABS_ENCRYPT_ACCOUNT_DATA"
    ...
    fun encryptAccountData(): Boolean {
        return defaultPrefs.getBoolean(SETTINGS_LABS_ENCRYPT_ACCOUNT_DATA, true)
    }
    ...
    // Defaults must match DefaultLightweightSettingsStorage, which is what the send path reads.
    fun getLinkPreviewSource(userId: String, isEncrypted: Boolean): LinkPreviewSource {
        return LinkPreviewSource.fromValue(defaultPrefs.getString(accountLinkPreviewKey(userId, isEncrypted), null))
                ?: if (isEncrypted) LinkPreviewSource.DEVICE else LinkPreviewSource.SERVER
    }

    fun setLinkPreviewSource(userId: String, isEncrypted: Boolean, source: LinkPreviewSource) {
        defaultPrefs.edit { putString(accountLinkPreviewKey(userId, isEncrypted), source.value) }
    }

    private fun accountLinkPreviewKey(userId: String, isEncrypted: Boolean) =
            "${if (isEncrypted) SETTINGS_LINK_PREVIEW_ENCRYPTED_KEY else SETTINGS_LINK_PREVIEW_UNENCRYPTED_KEY}_$userId"

    /** null means the room follows the account-wide setting for its kind of room. */
    fun getRoomLinkPreviewOverride(userId: String, roomId: String): LinkPreviewSource? {
        return LinkPreviewSource.fromValue(defaultPrefs.getString(roomLinkPreviewKey(userId, roomId), null))
    }

    fun setRoomLinkPreviewOverride(userId: String, roomId: String, source: LinkPreviewSource?) {
        val key = roomLinkPreviewKey(userId, roomId)
        defaultPrefs.edit {
            if (source == null) remove(key) else putString(key, source.value)
        }
    }

    private fun roomLinkPreviewKey(userId: String, roomId: String) = "${SETTINGS_LINK_PREVIEW_SOURCE_KEY}_${userId}_$roomId"
    ...
    fun arePerMessageProfilesEnabled(): Boolean {
        return defaultPrefs.getBoolean(SETTINGS_LABS_ENABLE_PER_MESSAGE_PROFILES, true)
    }
